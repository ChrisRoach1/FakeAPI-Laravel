...
new class extends Component {
    ...
    public function mount($mockTable)
    {
        $this->$mockTable = $mockTable;
        $this->tableName = $mockTable->name;

        foreach ($mockTable->mockFields as $mockField) {
            $this->fields[] = (object)['name' => $mockField->name, 'generator' => $mockField->generator];
        }
...
//        ]);
//
        $this->mockTable->update(['name' => $this->tableName]);
        $this->mockTable->mockFields()->delete();

        foreach ($this->fields as $field){
            if($field->name != '' && $field->generator != ''){
                $this->mockTable->mockFields()->create([
                    'name' => $field->name,
    ...
    <x-mary-form wire:submit="save">
        ...
        @foreach($fields as $key => $field)
            <div class="flex gap-x-4 items-end">
                ...
                <x-mary-select label="Type" :options="$options" wire:model="fields.{{$key}}.generator"
                               placeholder="Select a random value type" placeholder-value=""/>
